...
void PlayerUpdate(AInteractComponent* _aitc, Timer& _timer)
{
  ...
  auto Enemy1 = _aitc->GetActorOwner()->GetSceneNode().GetActorObject("enemy01-actor");
  auto Enemy2 = _aitc->GetActorOwner()->GetSceneNode().GetActorObject("enemy02-actor");

  if (!Enemy1 && !Enemy2)
  {
    _aitc->GetActorOwner()->GetAComponent<ATimerComponent>(COMP_TYPE::A_TIMER)->StartTimer("to-result");
    if (_aitc->GetActorOwner()->GetAComponent<ATimerComponent>(COMP_TYPE::A_TIMER)->GetTimer("to-result")->IsGreaterThan(3.f))
    {
      P_LOG(LOG_DEBUG, "to result scene\n");
      _aitc->GetActorOwner()->GetSceneNode().GetSceneManager()->LoadSceneNode("result-scene", "result-scene.json");
    }
  }
}
...
void ButtonInput(UInputComponent* _uic, Timer& _timer)
{
  ...
  if (ubc->IsCursorOnBtn() && InputInterface::IsKeyPushedInSingle(M_LEFTBTN))
  {
    ...
    else if (btnName == "title-ui-button")
    {
      _uic->GetUiOwner()->GetSceneNode().GetSceneManager()->LoadSceneNode("title-scene", "title-scene.json");
    }
    else if (btnName == "start-ui-button")
    {
      _uic->GetUiOwner()->GetSceneNode().GetSceneManager()->LoadSceneNode("gm02-scene", "gm02-scene.json");
    }
  }
